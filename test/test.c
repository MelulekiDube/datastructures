...
void insert_htest(){
	...
	prepend(list,  (DATA){ .data_int=0});
	CU_ASSERT_EQUAL(list->head, list->tail);
	prepend(list,  (DATA){ .data_int=1 });
	CU_ASSERT_EQUAL(list->head->data.data_int, 1);
	CU_ASSERT_EQUAL(list->length, 2);
	for(int i=2;i<10;++i){
		prepend(list, (DATA){.data_int=i });
...
void delete_test(){
	...
	CU_ASSERT_EQUAL(list->head, list->tail);
	CU_ASSERT_TRUE(del_node(list, (DATA){ .data_int=9 }, &compare));
	CU_ASSERT_PTR_NULL(list->head);
...
void sort_test(){
	...
	for(int i=0; i<7; ++i){
		insert(list,(DATA){.data_int=data[i]});
	}
	sort(list, &compare);
